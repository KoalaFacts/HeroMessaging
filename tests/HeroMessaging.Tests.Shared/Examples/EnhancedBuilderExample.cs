...
namespace HeroMessaging.Tests.TestUtilities.Examples;
        ...
        builder.During(awaitingDiscount)
            .When(discountApplied)
                .Then(ctx =>
                {
                    ...
                    ctx.Instance.DiscountAmount =
                        ctx.Instance.IsPremiumCustomer && ctx.Data.DiscountAmount > 0
                            ? ctx.Data.DiscountAmount * 1.1m
                            : ctx.Data.DiscountAmount;
                    ctx.Instance.DiscountCode = ctx.Data.DiscountCode;
                    return Task.CompletedTask;
                })
                .TransitionTo(awaitingPayment);
        ...
        builder.InState("AwaitingPayment")
            .When(new Event<PaymentAuthorizedEvent>("PaymentAuthorized"))
                .Then(ctx =>
                {
                    ...
                    ctx.Instance.CurrentState = ctx.Instance.TotalAmount > 1000
                        ? "RequiresApproval"
                        : ctx.Instance.IsPremiumCustomer
                            ? "FastTrackCompleted"
                            : "Completed";
