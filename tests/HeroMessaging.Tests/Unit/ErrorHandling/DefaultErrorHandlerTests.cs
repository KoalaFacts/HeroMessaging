...
namespace HeroMessaging.Tests.Unit.ErrorHandling
{
    ...
    public sealed class DefaultErrorHandlerTests
    {
        ...
        public async Task HandleErrorAsync_WithTransientErrorAndRetriesRemaining_ReturnsRetry()
        {
            ...
            // Act
            var result = await handler.HandleErrorAsync(message, error, context, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task HandleErrorAsync_WithMaxRetriesExceeded_IncludesCorrectContext()
        {
            ...
            // Act
            await handler.HandleErrorAsync(message, error, context, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task HandleErrorAsync_WithIncreasedRetryCount_IncreaseDelays()
        {
            ...
            var result1 = await handler.HandleErrorAsync(message, error, new ErrorContext { RetryCount = 0, MaxRetries = 5 }, cancellationToken: TestContext.Current.CancellationToken);
            var result2 = await handler.HandleErrorAsync(message, error, new ErrorContext { RetryCount = 1, MaxRetries = 5 }, cancellationToken: TestContext.Current.CancellationToken);
            var result3 = await handler.HandleErrorAsync(message, error, new ErrorContext { RetryCount = 2, MaxRetries = 5 }, cancellationToken: TestContext.Current.CancellationToken);
