...
public sealed class MessageProcessingPipelineTests : IDisposable
{
    ...
    public async Task Build_WithLogging_AddsLoggingDecorator()
    {
        ...
        var result = await pipeline.ProcessAsync(message, context, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CoreMessageProcessor_WithSuccessfulFunc_ReturnsSuccess()
    {
        ...
        // Act
        var result = await processor.ProcessAsync(message, context, cancellationToken: TestContext.Current.CancellationToken);
