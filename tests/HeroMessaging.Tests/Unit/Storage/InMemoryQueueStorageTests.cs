...
public sealed class InMemoryQueueStorageTests
{
    ...
    public async Task CreateQueueAsync_WithNewQueueName_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.CreateQueueAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CreateQueueAsync_WithExistingQueueName_ReturnsFalse()
    {
        ...
        var timeProvider = new FakeTimeProvider();
        var storage = new InMemoryQueueStorage(timeProvider);
        await storage.CreateQueueAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CreateQueueAsync_WithOptions_StoresOptions()
    {
        ...
        // Act
        await storage.CreateQueueAsync("test-queue", options, cancellationToken: TestContext.Current.CancellationToken);

        // Assert - Verify queue exists
        var exists = await storage.QueueExistsAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
        Assert.True(exists);
    }
    ...
    public async Task DeleteQueueAsync_WithExistingQueue_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.DeleteQueueAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DeleteQueueAsync_WithNonExistentQueue_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.DeleteQueueAsync("non-existent-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DeleteQueueAsync_RemovesQueueAndMessages()
    {
        ...
        await storage.CreateQueueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);

        // Act
        await storage.DeleteQueueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);

        // Assert
        var exists = await storage.QueueExistsAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
        Assert.False(exists);
    }
    ...
    public async Task GetQueuesAsync_WithNoQueues_ReturnsEmpty()
    {
        ...
        // Act
        var queues = await storage.GetQueuesAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetQueuesAsync_WithMultipleQueues_ReturnsAllQueueNames()
    {
        ...
        await storage.CreateQueueAsync("queue1", cancellationToken: TestContext.Current.CancellationToken);
        await storage.CreateQueueAsync("queue2", cancellationToken: TestContext.Current.CancellationToken);
        await storage.CreateQueueAsync("queue3", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task QueueExistsAsync_WithNonExistentQueue_ReturnsFalse()
    {
        ...
        // Act
        var exists = await storage.QueueExistsAsync("non-existent-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnqueueAsync_WithNewQueue_CreatesQueueAndEnqueuesMessage()
    {
        ...
        // Act
        var entry = await storage.EnqueueAsync(queueName, message, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnqueueAsync_WithDelay_SetsVisibleAtInFuture()
    {
        ...
        // Act
        var entry = await storage.EnqueueAsync("test-queue", message, options, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnqueueAsync_GeneratesUniqueIds()
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            var entry = await storage.EnqueueAsync("test-queue", new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
            ids.Add(entry.Id);
        }
    ...
    public async Task DequeueAsync_WithNonExistentQueue_ReturnsNull()
    {
        ...
        // Act
        var entry = await storage.DequeueAsync("non-existent-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_WithEmptyQueue_ReturnsNull()
    {
        ...
        // Act
        var entry = await storage.DequeueAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_WithVisibleMessage_ReturnsMessage()
    {
        ...
        await storage.EnqueueAsync(queueName, message, cancellationToken: TestContext.Current.CancellationToken);

        // Act
        var entry = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_SetsVisibleAtWithVisibilityTimeout()
    {
        ...
        await storage.CreateQueueAsync(queueName, new QueueOptions { VisibilityTimeout = visibilityTimeout }, cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_WithDelayedMessage_ReturnsNull()
    {
        ...
        await storage.EnqueueAsync(queueName, new TestMessage(), new EnqueueOptions { Delay = delay }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_WithHigherPriority_ReturnsHigherPriorityFirst()
    {
        ...
        await storage.EnqueueAsync(queueName, message1, new EnqueueOptions { Priority = 1 }, cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, message2, new EnqueueOptions { Priority = 10 }, cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, message3, new EnqueueOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_WithSamePriority_ReturnsFIFO()
    {
        ...
        await storage.EnqueueAsync(queueName, message1, new EnqueueOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));
        await storage.EnqueueAsync(queueName, message2, new EnqueueOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));
        await storage.EnqueueAsync(queueName, message3, new EnqueueOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);

        // Act
        var entry1 = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
        var entry2 = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
        var entry3 = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task DequeueAsync_WhenExceedsMaxDequeueCount_DoesNotReturnMessage()
    {
        ...
        await storage.CreateQueueAsync(queueName, new QueueOptions { MaxDequeueCount = maxDequeueCount }, cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
        ...
        var finalEntry = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_WithNonExistentQueue_ReturnsEmpty()
    {
        ...
        // Act
        var entries = await storage.PeekAsync("non-existent-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_WithEmptyQueue_ReturnsEmpty()
    {
        ...
        // Act
        var entries = await storage.PeekAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_WithVisibleMessages_ReturnsMessages()
    {
        ...
        // Act
        var entries = await storage.PeekAsync(queueName, count: 2, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_DoesNotIncrementDequeueCount()
    {
        ...
        // Act
        var entries = await storage.PeekAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_DoesNotChangeVisibleAt()
    {
        ...
        var originalEntry = await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
        var originalVisibleAt = originalEntry.VisibleAt;

        // Act
        await storage.PeekAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_RespectsCount()
    {
        ...
        // Act
        var entries = await storage.PeekAsync(queueName, count: 5, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_OrdersByPriorityThenEnqueuedAt()
    {
        ...
        // Act
        var entries = await storage.PeekAsync(queueName, count: 3, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task PeekAsync_ExcludesDelayedMessages()
    {
        ...
        await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
        await storage.EnqueueAsync(queueName, new TestMessage(), new EnqueueOptions { Delay = TimeSpan.FromMinutes(10) }, cancellationToken: TestContext.Current.CancellationToken);

        // Act
        var entries = await storage.PeekAsync(queueName, count: 10, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AcknowledgeAsync_WithNonExistentQueue_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.AcknowledgeAsync("non-existent-queue", "entry-id", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AcknowledgeAsync_WithNonExistentEntry_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.AcknowledgeAsync("test-queue", "non-existent-id", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AcknowledgeAsync_WithValidEntry_ReturnsTrue()
    {
        ...
        var entry = await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);

        // Act
        var result = await storage.AcknowledgeAsync(queueName, entry.Id, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AcknowledgeAsync_RemovesEntryFromQueue()
    {
        ...
        var entry = await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
        await storage.AcknowledgeAsync(queueName, entry.Id, cancellationToken: TestContext.Current.CancellationToken);

        // Act
        var depth = await storage.GetQueueDepthAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RejectAsync_WithNonExistentQueue_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.RejectAsync("non-existent-queue", "entry-id", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RejectAsync_WithNonExistentEntry_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.RejectAsync("test-queue", "non-existent-id", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RejectAsync_WithRequeueTrue_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.RejectAsync(queueName, entry.Id, requeue: true, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RejectAsync_WithRequeueFalse_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.RejectAsync(queueName, entry.Id, requeue: false, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RejectAsync_WithRequeueTrue_MakesMessageVisible()
    {
        ...
        var entry = await storage.EnqueueAsync(queueName, new TestMessage(), cancellationToken: TestContext.Current.CancellationToken);
        var dequeuedEntry = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
        Assert.NotNull(dequeuedEntry);

        // Act
        await storage.RejectAsync(queueName, entry.Id, requeue: true, cancellationToken: TestContext.Current.CancellationToken);

        // Assert - Message should be immediately visible
        var requeuedEntry = await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task RejectAsync_WithRequeueFalse_RemovesEntry()
    {
        ...
        // Act
        await storage.RejectAsync(queueName, entry.Id, requeue: false, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetQueueDepthAsync_WithNonExistentQueue_ReturnsZero()
    {
        ...
        // Act
        var depth = await storage.GetQueueDepthAsync("non-existent-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetQueueDepthAsync_WithEmptyQueue_ReturnsZero()
    {
        ...
        // Act
        var depth = await storage.GetQueueDepthAsync("test-queue", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetQueueDepthAsync_AfterDequeue_ExcludesInvisibleMessages()
    {
        ...
        await storage.DequeueAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnqueueAsync_WithMessageHavingMetadata_PreservesMetadata()
    {
        ...
        // Act
        var entry = await storage.EnqueueAsync("test-queue", message, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnqueueAsync_WithZeroDelay_MessageImmediatelyVisible()
    {
        ...
        await storage.EnqueueAsync(queueName, new TestMessage(), new EnqueueOptions { Delay = TimeSpan.Zero }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetQueueDepthAsync_AfterMessageBecomesVisible_IncludesMessage()
    {
        ...
        var depthBefore = await storage.GetQueueDepthAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);

        // Advance time
        timeProvider.Advance(TimeSpan.FromMinutes(6));

        // Act
        var depthAfter = await storage.GetQueueDepthAsync(queueName, cancellationToken: TestContext.Current.CancellationToken);
