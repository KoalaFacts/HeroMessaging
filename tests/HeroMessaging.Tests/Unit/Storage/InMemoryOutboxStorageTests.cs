...
public sealed class InMemoryOutboxStorageTests
{
    ...
    public async Task AddAsync_WithValidMessage_ReturnsOutboxEntry()
    {
        ...
        // Act
        var entry = await storage.AddAsync(message, options, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AddAsync_GeneratesUniqueIds()
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            var entry = await storage.AddAsync(new TestMessage(), new OutboxOptions(), cancellationToken: TestContext.Current.CancellationToken);
            ids.Add(entry.Id);
        }
    ...
    public async Task GetPendingAsync_WithQuery_ReturnsOnlyPendingEntries()
    {
        ...
        await storage.MarkProcessedAsync(entry2.Id, cancellationToken: TestContext.Current.CancellationToken);

        var query = new OutboxQuery { Limit = 100 };

        // Act
        var results = await storage.GetPendingAsync(query, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_WithStatusFilter_ReturnsMatchingEntries()
    {
        ...
        var timeProvider = new FakeTimeProvider();
        var storage = new InMemoryOutboxStorage(timeProvider);
        _ = await storage.AddAsync(new TestMessage(), new OutboxOptions(), cancellationToken: TestContext.Current.CancellationToken);
        var entry2 = await storage.AddAsync(new TestMessage(), new OutboxOptions(), cancellationToken: TestContext.Current.CancellationToken);
        var entry3 = await storage.AddAsync(new TestMessage(), new OutboxOptions(), cancellationToken: TestContext.Current.CancellationToken);

        await storage.MarkProcessedAsync(entry2.Id, cancellationToken: TestContext.Current.CancellationToken);
        await storage.MarkFailedAsync(entry3.Id, "Test error", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_ExcludesEntriesWithFutureNextRetry()
    {
        ...
        await storage.UpdateRetryCountAsync(entry2.Id, 1, futureRetry, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_IncludesEntriesWithPastNextRetry()
    {
        ...
        await storage.UpdateRetryCountAsync(entry2.Id, 1, pastRetry, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_OrdersByPriorityThenCreatedAt()
    {
        ...
        var entry1 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));

        var entry2 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 10 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));

        var entry3 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));

        var entry4 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 10 }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_WithLimit_ReturnsOnlyPendingEntries()
    {
        ...
        // Act
        var results = await storage.GetPendingAsync(limit: 100, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_WithLimit_RespectsLimit()
    {
        ...
        // Act
        var results = await storage.GetPendingAsync(limit: 5, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_WithLimit_OrdersByPriorityThenCreatedAt()
    {
        ...
        var entry1 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 1 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));
        var entry2 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 5 }, cancellationToken: TestContext.Current.CancellationToken);
        timeProvider.Advance(TimeSpan.FromSeconds(1));
        var entry3 = await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 1 }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MarkProcessedAsync_WithExistingEntry_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.MarkProcessedAsync(entry.Id, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MarkProcessedAsync_WithNonExistentEntry_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.MarkProcessedAsync("non-existent-id", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MarkProcessedAsync_SetsProcessedAtTimestamp()
    {
        ...
        // Act
        await storage.MarkProcessedAsync(entry.Id, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MarkFailedAsync_WithExistingEntry_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.MarkFailedAsync(entry.Id, "Test error", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MarkFailedAsync_WithNonExistentEntry_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.MarkFailedAsync("non-existent-id", "Error", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MarkFailedAsync_StoresErrorMessage()
    {
        ...
        // Act
        await storage.MarkFailedAsync(entry.Id, errorMessage, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_WithExistingEntry_ReturnsTrue()
    {
        ...
        // Act
        var result = await storage.UpdateRetryCountAsync(entry.Id, 3, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_WithNonExistentEntry_ReturnsFalse()
    {
        ...
        // Act
        var result = await storage.UpdateRetryCountAsync("non-existent-id", 1, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_UpdatesRetryCount()
    {
        ...
        // Act
        await storage.UpdateRetryCountAsync(entry.Id, 5, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_SetsNextRetryAt()
    {
        ...
        // Act
        await storage.UpdateRetryCountAsync(entry.Id, 2, nextRetry, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_WhenExceedsMaxRetries_MarksAsFailed()
    {
        ...
        var entry = await storage.AddAsync(new TestMessage(), options, cancellationToken: TestContext.Current.CancellationToken);

        // Act
        await storage.UpdateRetryCountAsync(entry.Id, 3, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_WhenBelowMaxRetries_KeepsPendingStatus()
    {
        ...
        // Act
        await storage.UpdateRetryCountAsync(entry.Id, 2, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingCountAsync_WithNoPendingEntries_ReturnsZero()
    {
        ...
        // Act
        var count = await storage.GetPendingCountAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingCountAsync_IncludesFailedEntries()
    {
        ...
        await storage.MarkFailedAsync(entry2.Id, "Error", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetFailedAsync_WithNoFailedEntries_ReturnsEmpty()
    {
        ...
        // Act
        var results = await storage.GetFailedAsync(cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetFailedAsync_WithFailedEntries_ReturnsFailedOnly()
    {
        ...
        await storage.MarkFailedAsync(entry3.Id, "Error 2", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetFailedAsync_RespectsLimit()
    {
        ...
        {
            var entry = await storage.AddAsync(new TestMessage(), new OutboxOptions(), cancellationToken: TestContext.Current.CancellationToken);
            await storage.MarkFailedAsync(entry.Id, $"Error {i}", cancellationToken: TestContext.Current.CancellationToken);
        }

        // Act
        var results = await storage.GetFailedAsync(limit: 5, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentAddAndGet_WorksCorrectly()
    {
        ...
        var pending = await storage.GetPendingAsync(limit: 200, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ConcurrentUpdateRetryCount_WorksCorrectly()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            var entry = await storage.AddAsync(new TestMessage(), new OutboxOptions { MaxRetries = 10 }, cancellationToken: TestContext.Current.CancellationToken);
            entries.Add(entry);
        }
        ...
        var pending = await storage.GetPendingAsync(limit: 100, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AddAsync_WithMessageHavingMetadata_StoresCorrectly()
    {
        ...
        // Act
        var entry = await storage.AddAsync(message, new OutboxOptions(), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task UpdateRetryCountAsync_WithNullNextRetry_UpdatesOnlyRetryCount()
    {
        ...
        // Act
        await storage.UpdateRetryCountAsync(entry.Id, 2, null, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_WithZeroPriority_WorksCorrectly()
    {
        ...
        await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 0 }, cancellationToken: TestContext.Current.CancellationToken);
        await storage.AddAsync(new TestMessage(), new OutboxOptions { Priority = 0 }, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetPendingAsync_WithNextRetryAtCurrentTime_IncludesEntry()
    {
        ...
        await storage.UpdateRetryCountAsync(entry.Id, 1, currentTime, cancellationToken: TestContext.Current.CancellationToken);
