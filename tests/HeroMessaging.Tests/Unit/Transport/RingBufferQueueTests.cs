...
public class RingBufferQueueTests
{
    ...
    public async Task EnqueueAsync_SingleMessage_IncrementsCounters()
    {
        ...
        // Act
        var result = await queue.EnqueueAsync(envelope, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnqueueAsync_MultipleMessages_IncrementsCounters()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            var result = await queue.EnqueueAsync(CreateTestEnvelope(), cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(result);
        }
    ...
    public async Task AddConsumer_AndEnqueue_DeliversMessageToConsumer()
    {
        ...
        // Act
        queue.AddConsumer(consumer);
        await queue.EnqueueAsync(CreateTestEnvelope("TestMessage"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AddConsumer_MultipleConsumers_DistributesMessagesRoundRobin()
    {
        ...
        await consumer1.StartAsync(cancellationToken: TestContext.Current.CancellationToken);
        await consumer2.StartAsync(cancellationToken: TestContext.Current.CancellationToken);
        await consumer3.StartAsync(cancellationToken: TestContext.Current.CancellationToken);
        ...
        for (int i = 0; i < 9; i++)
        {
            await queue.EnqueueAsync(CreateTestEnvelope($"Message{i}"), cancellationToken: TestContext.Current.CancellationToken);
        }
    ...
    public async Task RemoveConsumer_AfterRemoval_StopsDeliveringMessages()
    {
        ...
        await queue.EnqueueAsync(CreateTestEnvelope("AfterRemoval1"), cancellationToken: TestContext.Current.CancellationToken);
        await queue.EnqueueAsync(CreateTestEnvelope("AfterRemoval2"), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task MessageDepth_AfterDelivery_Decrements()
    {
        ...
        await queue.EnqueueAsync(CreateTestEnvelope(), cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task HighThroughput_ManyMessages_ProcessesAllCorrectly()
    {
        ...
        {
            tasks.Add(queue.EnqueueAsync(CreateTestEnvelope($"Message{i}"), cancellationToken: TestContext.Current.CancellationToken).AsTask());
        }
    ...
    public async Task ConsumerFailure_DoesNotStopQueue()
    {
        ...
        await queue.EnqueueAsync(CreateTestEnvelope("SuccessMessage1"), cancellationToken: TestContext.Current.CancellationToken);
        await queue.EnqueueAsync(CreateTestEnvelope("FailMessage"), cancellationToken: TestContext.Current.CancellationToken);
        await queue.EnqueueAsync(CreateTestEnvelope("SuccessMessage2"), cancellationToken: TestContext.Current.CancellationToken);
